...
Support files:
- `conf/database.sql`: MySQL schema initialization.
- `log/`: runtime log directory placeholder.

## Build, Test, and Development Commands
- `python3 -m venv .venv && source .venv/bin/activate`: create local virtualenv.
- `pip install -r requirements.txt`: install dependencies (keep `aiohttp` on `3.8.5`; `3.9.1` is known to break request handling).
- `mysql -u <user> -p < conf/database.sql`: create/reset schema.
...
- Validate DB-impacting changes against a fresh schema from `conf/database.sql`.
